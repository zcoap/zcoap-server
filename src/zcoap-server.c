...
static void print_ct(const coap_node_t * const node, size_t *len, char **buf, size_t *remain)
{
    ...
    if (mask.ct_bool) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_BOOL);
    }
    if (mask.ct_u16) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_U16);
    }
    if (mask.ct_u32) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_U32);
    }
    if (mask.ct_u64) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_U64);
    }
    if (mask.ct_i16) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_I16);
    }
    if (mask.ct_i32) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_I32);
    }
    if (mask.ct_i64) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_I64);
    }
    if (mask.ct_float) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_FLOAT);
    }
    if (mask.ct_double) {
        SNPRINTF(len, buf, remain, ";ct=%u", ZEPTO_FMT_DOUBLE);
    }
 ...
 */
static href_filter_t get_href_filter(size_t nopts, coap_msg_opt_t opts[])
{
    /* Per RFC6690:
     *
 ...
 */
static uint8_t *stuff_option(uint32_t *acc, uint16_t optnum, uint16_t optlen, const void *opt, uint8_t *buf)
{
 ...
 */
void __attribute__((nonnull (1))) coap_rsp(coap_req_data_t *req, coap_code_t code, size_t nopts, const coap_opt_t opts[], size_t pl_len, void *payload)
{
    // Check arguments.
    if (   req == NULL
        || req->msg == NULL
 ...
 */
void __attribute__((nonnull (1))) coap_content_rsp(coap_req_data_t *req, coap_code_t code, coap_ct_t ct, size_t pl_len, const void *payload)
{
 ...
 * @return 0 on success, non-zero on error
 */
void __attribute__((nonnull (1))) coap_detail_rsp(coap_req_data_t *req, coap_code_t code, const char *detail)
{
    coap_rsp(req, code, COAP_FMT_TEXT, strlen(detail), detail);
}
 ...
 * @return 0 on success, non-zero on error
 */
void __attribute__((nonnull (1))) coap_status_rsp(coap_req_data_t *req, coap_code_t code)
{
 ...
 */
static uint8_t opt_next(uint8_t **buf, size_t *remain, uint32_t *num, uint16_t *len, void **val)
{
    if (!*remain) {
        return 0;
    }
 ...
 * @return 0 on success, else a CoAP error code
 */
static coap_code_t __attribute__((nonnull (1, 2))) coap_count_opts(coap_req_data_t *req, size_t *nopts)
{
    ...
    *nopts = 0;
    ...
    if (*nopts > MAX_COAP_PAYLOAD_OPTS) {
        return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }  else {
        return 0;
    }
 ...
 * @return 0 on success, else a CoAP error code
 */
static uint8_t __attribute__((nonnull (1, 3))) coap_get_opts(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[])
{
 ...
 */
coap_code_t __attribute__((nonnull (1, 4))) coap_get_content_type(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], coap_ct_t *ct)
{
    if (ct == NULL) {
        return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }
    ...
    *ct = COAP_FMT_NONE;
    ...
    if (opts == NULL) {
        ...
        cf_opt = bsearch(&key, lopts, nopts, sizeof(lopts[0]), &opt_cmp);
    } else {
        const coap_msg_opt_t key = { .num = COAP_OPT_CONTENT_FMT };
        cf_opt = bsearch(&key, opts, nopts, sizeof(opts[0]), &opt_cmp);
    }
    if (cf_opt == NULL) {
    ...
    if (cf_opt->len > sizeof(coap_ct_t)) {
        ...
        // Reject!
        return COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_BAD_OPT);
    } else if (cf_opt->len == sizeof(coap_ct_t)) {
        uint16_t netshort;
        ZCOAP_MEMCPY(&netshort, cf_opt->val, sizeof(netshort));
        *ct = ntoh(netshort);
    } else {
        *ct = *(uint8_t *)cf->val;
    }
 ...
 */
coap_code_t __attribute__((nonnull (1, 2, 3))) coap_get_payload(coap_req_data_t *req, size_t *len, void **payload)
{
 ...
 */
#define GET_CONTENT_TYPE_AND_PAYLOAD(_req) ({\
    coap_code_t _rc;\
    if ((_rc = coap_get_content_type(req, nopts, opts, &ct))) {\
        return rc;\
    }\
    if ((rc = coap_get_payload(req, &len, &payload))) {\
        return rc;\
    }\
})
 ...
 * @return 0 on success, else a CoAP error code
 */
static coap_code_t __attribute__((nonnull (1, 4))) process_req_uri(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const coap_node_t * const node)
{
    ...
    size_t len;
    void *payload;
    switch (req->msg->code.class) {
        case COAP_REQ:
            switch (req->msg->code.detail) {
                ...
                default:
                    #ifdef ZCOAP_DEBUG
                    ZCOAP_DEBUG("%s: unable to service method %u for path '%s'", __func__, req->msg->code.detail, node->name);
                    #endif
                    return COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_METHOD_NOT_ALLOWED);
            }
        default:
            #ifdef ZCOAP_DEBUG
            ZCOAP_DEBUG("%s: ignoring message with class %u and path option '%s'", __func__, req->msg->code.class, node->name);
            #endif
            return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }
}
...
// Forward declaration.
static coap_code_t __attribute__((nonnull (1, 3, 5, 6))) iter_req_uri(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], size_t npath_opts, coap_msg_opt_t *path_opts, const coap_node_t * const path);
...
static coap_code_t __attribute__((nonnull (1, 3, 5, 6))) iter_req_uri(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], size_t npath_opts, coap_msg_opt_t *path_opts, const coap_node_t * const path)
{
    ...
    coap_msg_opt_t *opt = &path_opts[0];
    if (count && opt->len < MAX_ZCOAP_BUF_SIZE) { // skip path segments that are too large
        ...
        if (c) {
            ...
            node.parent = path;
            if (node.parent->lockout_exempt) {
                node.lockout_exempt = true; // inherit parent designation
            }
            --npath_opts;
    ...
    for (const coap_gen_t *g = path->gens; g != NULL && *g != NULL; ++g) {
        ...
        while (1) {
            ...
            if (   !strncmp((char *)opt->val, dynamic_node.name, opt->len)
                ...
                if (dynamic_node.parent->lockout_exempt) {
                    dynamic_node.lockout_exempt = true; // inherit parent designation
                }
                --npath_opts;
                if (!npath_opts) { // end of options
 ...
 */
static void __attribute__((nonnull (1, 1))) inject_coap_req(coap_req_data_t *req, coap_node_t *root)
{
    size_t nopts;
    coap_code_t rc;
    // Count options.
    ...
    if (a_path_opt == NULL) {
        ...
        process_req_uri(req, nopts, opts, &root_uri);
    }
 ...
 */
void coap_rx(coap_req_data_t *req, coap_node_t *root)
{
 ...
 * @return 0 on success, else a CoAP error code
 */
coap_code_t __attribute__((nonnull (1, 4))) coap_count_query_opts(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], size_t *nqueryopts)
{
 ...
 * @return 0 on success, else a CoAP error code
 */
coap_code_t __attribute__((nonnull (1, 5))) coap_get_query_opts(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], size_t nqueryopts, coap_msg_opt_t queryopts[])
{
    if (nqueryopts == 0) {
        return 0;
    }
 ...
 */
extern uint8_t __attribute__((nonnull (4, 5))) coap_get_size1(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], bool *found, uint32_t *size1)
{
 ...
 * @param size1 (out) written with max buffer size if an ENOMEM error occurs and size1 is non-null
 * @return 0 on success, an appropriate errno on error
 */
static int coap_parse_ullong(void *ascii, size_t len, unsigned long long *out, size_t *size1)
{
    if (len >= MAX_ZCOAP_BUF_SIZE) {
        if (size1) {
            *size1 = MAX_ZCOAP_BUF_SIZE - 1;
        }
        return ENOMEM;
    }
    char buf[len + 1];
 ...
 * @return 0 on success, an appropriate errno on error
 */
static int coap_parse_llong(void *ascii, size_t len, long long *out, size_t *size1)
{
    if (len >= MAX_ZCOAP_BUF_SIZE) {
        if (size1) {
            *size1 = MAX_ZCOAP_BUF_SIZE - 1;
 ...
 * @return 0 on success, an appropriate errno on error
 */
int coap_parse_ulong(void *ascii, size_t len, unsigned long *out, size_t *size1)
{
    if (len >= MAX_ZCOAP_BUF_SIZE) {
        if (size1) {
            *size1 = MAX_ZCOAP_BUF_SIZE - 1;
 ...
 * @return 0 on success, an appropriate errno on error
 */
int coap_parse_long(void *ascii, size_t len, long *out, size_t *size1)
{
    if (len >= MAX_ZCOAP_BUF_SIZE) {
        if (size1) {
            *size1 = MAX_ZCOAP_BUF_SIZE - 1;
 ...
 * @return 0 on success, an appropriate errno on error
 */
int coap_parse_uint(void *ascii, size_t len, unsigned *out, size_t *size1)
{
    int rc;
    unsigned long ulong;
    if ((rc = coap_parse_ulong(ascii, len, &ulong, size1))) {
        return rc;
    }
    if (ulong > UINT_MAX) {
        return EINVAL;
 ...
 * @return 0 on success, an appropriate errno on error
 */
int coap_parse_int(void *ascii, size_t len, int *out, size_t *size1)
{
    int rc;
    long slong;
    if ((rc = coap_parse_long(ascii, len, &slong, size1))) {
        return rc;
    }
 ...
 * @return 0 on success, an appropriate errno on error
 */
int coap_parse_float(void *ascii, size_t len, float *out, size_t *size1)
{
    if (len >= MAX_ZCOAP_BUF_SIZE) {
        if (size1) {
            *size1 = MAX_ZCOAP_BUF_SIZE - 1;
    ...
    *out = strtof(buf, &endptr);
    if (!endptr || endptr == buf) {
        return EINVAL;
    }  else if (zmantf(*out) == 0 && zexponentf(*out) == 1) {
        // Unfortunately, our c library strtof does not seem to set errno.
        // Instead, for both overflow and underflow, it returns the smallest
        // normalized positive number.  We can key off of this to for quick
        // over and underflow indication.
        return ERANGE;
    }
    return 0;
}
...
/**
 * Parse a unsigned 64-bit integer from a CoAP request payload.
 * Return an appropriate CoAP error code on error.
 *
 * @param req request for which to attempt payload parse
...
coap_code_t coap_parse_req_u64(coap_ct_t ct, size_t len, void *payload, uint64_t *out)
{
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_I16: {
            ...
            pval = ntoh(pval);
            if (pval < 0) {
                rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_BAD_REQ);
                break;
            }
            *out = pval;
            return 0;
...
/**
 * Parse long long int from a plain text CoAP payload.  Transmit an appropriate
 * CoAP error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 * @param opts options array, or NULL if unknown to the caller; if NULL, options will be parsed from req
...
coap_code_t __attribute__((nonnull (1, 4))) coap_parse_req_llong(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], long long *out)
{
    ...
    uint8_t *content_fmt = NULL;
    if ((rc = coap_get_content_type(req, nopts, opts, &content_fmt))) {
        coap_status_rsp(req, rc);
        return rc;
    }
    size_t len;
    ...
    if (content_fmt) {
        switch (*content_fmt) {
            ...
            case COAP_FMT_TEXT:
                break; // handled below
            default:
                rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_CONTENT_FMT);
                break;
        }
    }
    if (rc) {
    ...
    int ec;
    size_t size1;
    if ((ec = coap_parse_llong(payload, len, out, &size1))) {
        switch (ec) {
            case ENOMEM:
                rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_REQ_TOO_LARGE);
                coap_client_err_req_too_large(req, size1);
                return rc;
            default:
                rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_BAD_REQ);
                coap_status_rsp(req, rc);
...
/**
 * Parse an unsigned long from a type-variant CoAP payload.  Transmit an
 * appropriate CoAP error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 * @param opts options array, or NULL if unknown to the caller; if NULL, options will be parsed from req
...
/**
 * Parse a long int from a type-variant CoAP payload.  Transmit an appropriate
 * CoAP error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 ...
 * @return 0 on success, non-zero CoAP status code (4.00-class or 5.00-class) on failure
 */
coap_code_t __attribute__((nonnull (1, 4))) coap_parse_req_long(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], long *out)
{
    if (req == NULL) {
        return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }
...
/**
 * Parse a float from a plain text CoAP payload.  Transmit an appropriate CoAP
 * error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 * @param opts options array, or NULL if unknown to the caller; if NULL, options will be parsed from req
 ...
 * @return 0 on success, non-zero CoAP status code (4.00-class or 5.00-class) on failure
 */
coap_code_t __attribute__((nonnull (1, 4))) coap_parse_req_float(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], float *out)
{
    if (req == NULL) {
        return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }
...
// Shallow wrappers for the ulong and long parsers

/**
 * Parse an unsigned int from a plain text CoAP payload.  Transmit an
 * appropriate CoAP error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 ...
 * @return 0 on success, non-zero CoAP status code (4.00-class or 5.00-class) on failure
 */
coap_code_t __attribute__((nonnull (1, 4))) coap_parse_req_uint(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], unsigned *out)
{
    coap_code rc;
    unsigned long ulong;
    if ((rc = coap_parse_req_ulong(req, nopts, opts, &ulong))) { // issues response and calls discard on our behalf if a parse error occurs
        return  rc;
    }
    if (ulong > UINT_MAX) {
        rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_BAD_REQ);
...
/**
 * Parse an int from a plain text CoAP payload.  Transmit an appropriate CoAP
 * error status response and free req on error.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 ...
 * @return 0 on success, non-zero CoAP status code (4.00-class or 5.00-class) on failure
 */
coap_code_t coap_parse_req_int(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], int *out)
{
    coap_code_t rc;
    long slong;
    if ((rc = coap_parse_req_long(req, nopts, opts, &slong))) { // issues response and calls discard on our behalf if a parse error occurs
        return  rc;
    }
    if (slong > INT_MAX || slong < INT_MIN) {
        rc = COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_BAD_REQ);
        coap_status_rsp(req, rc);
        return rc;
    } else {
...
/**
 * Parse a bool from a type-variant CoAP payload.  For plain text payloads, we
 * first match against strings 'true' and 'false'.  If we do not match, we
 * attempt to parse a numerical text payload and infer truthiness that way.
 * For binary encodings, we only accept type bool.
 *
 * @param req request for which to attempt payload parse
 * @param nopts number of options in the passed options array
 * @param opts options array, or NULL if unknown to the caller; if NULL, options will be parsed from req
 ...
 * @return 0 on success, non-zero CoAP status code (4.00-class or 5.00-class) on failure
 */
coap_code_t coap_parse_req_bool(coap_req_data_t *req, coap_ct_t ct, size_t len, void *payload, bool *out)
{
    if (req == NULL) {
        return COAP_CODE(COAP_SERVER_ERR, COAP_SERVER_ERR_INTERNAL);
    }
 ...
 * @param ... printf arguments
 */
void coap_printf(coap_req_data_t *req, const char *fmt, ...)
{
    va_list ap;
    size_t len;
    {
 ...
 */
void coap_return_bool(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], bool val)
{
    coap_ct_t ct;
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_BOOL: {
            ...
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_BOOL, sizeof(_val), &_val);
            break;
        }
        case ZEPTO_FMT_U16:
        case ZEPTO_FMT_I16: {
 ...
 * @param node server URI tree node
 */
void coap_return_u16(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, uint16_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
 ...
 * @param node server URI tree node
 */
void coap_return_u32(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, uint32_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_U32:
            val = ZCOAP_HTONL(val);
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_U32, sizeof(val), &val);
            break;
        case ZEPTO_FMT_U64:
        case ZEPTO_FMT_I64: {
            int64_t _val = val;
 ...
 * @param node server URI tree node
 */
void coap_return_u64(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, uint64_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_U64:
            ...
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_U64, sizeof(val), &val);
            break;
        #endif /* ZEPTO_COAP_EXTENSIONS */
        default:
            coap_status_rsp(req, COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_CONTENT_FMT));
 ...
 * @param node server URI tree node
 */
void coap_return_i16(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, int16_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
 ...
 * @param node server URI tree node
 */
void coap_return_i32(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, int32_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_I32:
            val = ZCOAP_HTONL(val);
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_I32, sizeof(val), &val);
            break;
        case ZEPTO_FMT_I64: {
            int64_t _val = val;
            _val = ZCOAP_HTONLL(_val);
 ...
 * @param node server URI tree node
 */
void coap_return_i64(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, int64_t val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_I64:
            val = ZCOAP_HTONLL(val);
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_I64, sizeof(val), &val);
            break;
        #endif /* ZEPTO_COAP_EXTENSIONS */
        default:
            coap_status_rsp(req, COAP_CODE(COAP_CLIENT_ERR, COAP_CLIENT_ERR_CONTENT_FMT));
 ...
 * @param node server URI tree node
 */
void coap_return_float(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, float val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
    ...
    switch (ct) {
        ...
        case ZEPTO_FMT_FLOAT:
            val = ZCOAP_HTONF(val);
            coap_content_rsp(req, COAP_CODE(COAP_SUCCESS, COAP_SUCCESS_CONTENT), ZEPTO_FMT_FLOAT, sizeof(val), &val);
            break;
        case ZEPTO_FMT_DOUBLE: {
            ZCOAP_DOUBLE _val = val;
            _val = ZCOAP_HTOND(_val);
 ...
 * @param node server URI tree node
 */
void coap_return_double(coap_req_data_t *req, size_t nopts, coap_msg_opt_t opts[], const char *fmt, ZCOAP_DOUBLE val)
{
    coap_ct_t ct;
    coap_code_t coap_code;
    if ((coap_code = coap_get_content_type(req, nopts, opts, &ct))) {
...
void coap_put_bool(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    bool val;
    if (coap_parse_req_bool(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
...
void coap_put_u16(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    uint16_t val;
    if (coap_parse_req_u16(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err = NULL;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_u32(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    uint32_t val;
    if (coap_parse_req_u32(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_u64(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    uint64_t val;
    if (coap_parse_req_u64(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_i16(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    int16_t val;
    if (coap_parse_req_i16(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_i32(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    int32_t val;
    if (coap_parse_req_int32(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err = NULL;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_i64(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    int64_t val;
    if (coap_parse_req_int64(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err = NULL;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_float(ZEPTO_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    float val;
    if (coap_parse_req_float(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err;
    if (node->validate && (err = (*node->validate)(&val))) {
...
void coap_put_double(ZCOAP_METHOD_SIGNATURE)
{
    ...
    ZCOAP_FMT_SENTINEL);
    ZCOAP_DOUBLE val;
    if (coap_parse_req_double(req, nopts, opts, &val)) { // issues response and calls discard on our behalf if a parse error occurs
        return;
    }
    const char *err;
    if (node->validate && (err = (*node->validate)(&val))) {
